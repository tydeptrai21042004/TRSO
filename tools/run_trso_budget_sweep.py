#!/usr/bin/env python3
"""Launch reproducible Scientific-TRSO kernel/rank/budget sweeps."""
...
def main():
    ...
    parser.add_argument("--parameter_budgets", default="0")
    ...
    budgets = csv_values(args.parameter_budgets, int)
    keep_ratios = csv_values(args.keep_ratios, float)
    seeds = csv_values(args.seeds, int)

    for kernel, rank, budget, keep_ratio, seed in itertools.product(
        kernels, ranks, budgets, keep_ratios, seeds
    ):
        if rank > kernel * kernel:
            continue
        run_name = f"k{kernel}_r{rank}_b{budget}_keep{keep_ratio:g}_seed{seed}"
